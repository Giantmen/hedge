...
func (j *EtcChBtctrade) checkAccount(sellSide, buySide account, priceS, pirceB *proto.Price, amount string) error {
	...
	if sellSide.etc < num {
		return fmt.Errorf("%s etc 余额不足:%f cny:%f", sellSide.name, sellSide.etc, sellSide.cny)
	} else if buySide.cny < (pirceB.Sell * num) {
		return fmt.Errorf("%s cny 余额不足:%f etc:%f", buySide.name, buySide.cny, buySide.etc)
	}
	return nil
}
...
func (j *EtcChBtctrade) hedging(sellSide, buySide account, priceS, pirceB *proto.Price, amount string) error {
	...
	if err != nil {
		log.Error(err)
		if order, err = j.retry(buySide.bourse, proto.BUY, amount); err != nil {
			return err //重试失败
		}
		log.Info("retry ok")
	}
...
func (j *EtcChBtctrade) retry(bou bourse.Bourse, side, amount string) (*proto.Order, error) {
	sec := rand.Intn(10)
	for {
